...
fn get_new_token(quiet: bool, config: &Option<Config>) -> BoxedResult<models::TokenResponse> {
    let email = config
        .clone()
...
pub fn get_token(quiet: bool) -> BoxedResult<models::TokenResponse> {
    ...
    if data.is_none() {
        ...
        Config::load(true);
        data = Some(get_new_token(quiet, &Some(config))?);
